...
public class EEMpresaInnovadora {
	...
	public static void main(String[] args) throws ParseException {
		...
		do {
			...
			case 2: System.out.print("Ingrese dni del cliente:");
			        ...
			        break;
			case 3: System.out.print("Ingrese dni del cliente:");
			        dni=sc.nextInt();
			        fecha = entregaF();
			        pos=buscDniPaq(paquetes,dni,fecha);
	...
	public static int buscDniPaq(Paquete []paquete , int dni, Date fecha) {
		int pos=0;
		for(int i=0;paquete[i]!=null&&i<paquete.length;i++) {
	...
	public static int buscDniPer(Persona []per , int dni) {
		int pos=0;
		for(int i=0;per[i]!=null&&i<per.length;i++) {
			if (per[i].getDni()==dni) {
				pos=i;
			}
		}		
    ...
    public static Paquete  ingPaq(int i, Persona []personas,int dni,Paquete []paquetes) throws ParseException {
		...
		Paquete paq= new Paquete(identificador,fechaEntrega,fechaRecepcion,peso, direccion,  costo,
				null);								
		return paq;
	}
